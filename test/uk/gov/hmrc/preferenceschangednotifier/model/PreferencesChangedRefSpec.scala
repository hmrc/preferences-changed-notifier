...
class PreferencesChangedRefSpec extends AnyFreeSpec {
  ...
  "Preferences changed ref json writes" - {
    ...
    "preferencesChangedId ObjectId writes correctly" in {
      ...
      val a = PreferencesChangedRef(preferenceChangedId, new ObjectId(), Some(entityId), "sub1")
    ...
    "preferencesId ObjectId writes correctly" in {
      ...
      val entityId = EntityId.generate()
      val a =
        PreferencesChangedRef(new ObjectId(), preferenceId, Some(entityId), "sub1")
    ...
    "subscriber string writes correctly" in {
      val entityId = EntityId.generate()
      val a = PreferencesChangedRef(new ObjectId(), new ObjectId(), Some(entityId), "sub1")
